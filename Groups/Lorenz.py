...
site = 'http://lorenzmlwpzgxq736jzseuterytjueszsvznuibanxomlpkyxk6ksoyd.onion'
driver.get(site)
...
name_elements = driver.find_elements("xpath", '//div[@class="panel-heading"]/h3')
for element in name_elements:
    company_names.append(element.text if element.text else 'n/a')

website_elements = driver.find_elements("xpath", "//div[@class='panel-heading']/h5/a[@style]")
for element in website_elements:
    company_website.append(element.text if element.text else 'n/a')

date_elements = driver.find_elements("xpath", '//div[@class="panel panel-primary"]')
for element in date_elements:
    Date.append(element.text if element.text else 'n/a')

Datas = driver.find_elements("xpath", '//div[@class="panel-body"]/a[1]')
for Data in Datas:
    Data_Leak.append(Data.get_attribute('href') if Data.get_attribute('href') else 'n/a')
...
for i in range(len(company_names)):
    if company_names[i] not in existing_company_names:
        data_dict = {
            ...
            'company_description': Date[i],
            'ransomware_name': 'Lorenz',
            'ransomware_site': 'http://lorenzmlwpzgxq736jzseuterytjueszsvznuibanxomlpkyxk6ksoyd.onion',
            'data_description': Date[i],
            'data_date': current_date,  # Use the modified current_date
            'download_data': Data_Leak[i],
            'company_website': Date[i],
            "group_url": "1",
            "title": "1",
            "status_code": 1,
            "is_active": False,
